...
class Face_Recognition:
    ...
    def __init__(self, root):
        ...
        # title section
        title_lb1 = Label(bg_img, text="Welcome to Face Recognition Pannel", font=("verdana", 30, "bold"), bg="white", fg="black")
    ...
    # =====================Attendance===================
    def mark_attendance(self,n,s,d):
        with open("attendance.csv","r+",newline="\n") as f:
            ...
            if((n not in name_list)) and ((s not in name_list)) and ((d not in name_list)):
                now=datetime.now()
                d1=now.strftime("%d/%m/%Y")
                dtString=now.strftime("%H:%M:%S")
                f.writelines(f"\n{n}, {s}, {d}, {dtString}, {d1}, Present")
    ...
    def face_recog(self):
        def draw_boundray(img, classifier, scaleFactor, minNeighbors, color, text, clf):
            ...
            for (x, y, w, h) in featuers:
                ...
                conn = mysql.connector.connect(username='root', password='Winston@123', host='localhost', database='face_recognition', port=3306)
                cursor = conn.cursor()
                ...
                if confidence > 77:
                    ...
                    cv2.putText(img, f"Student_ID:{s}", (x, y - 55), cv2.FONT_HERSHEY_COMPLEX, 0.8, (64, 15, 223), 2)
                    cv2.putText(img, f"Department:{d}", (x, y - 30), cv2.FONT_HERSHEY_COMPLEX, 0.8, (64, 15, 223), 2)
                    self.mark_attendance(n,s,d)
                else:
